...
（同批值重排）故單變數 3σ 盲，但變數間相關結構被破壞 → 多變量 SPE 升（X 健康抓到），且 X 與 Y 的配對
被打亂 → 軟測量 Ŷ 偏離實際 Y（Y 健康抓到）。實測：單變數 univ≈0.04（盲）、X-MSPC≈0.23、L2 健康
0.77 vs 乾淨 0.98。**這是真實 TEP 結構 + 受控注入飄移的半合成情境**，非宣稱真實故障隱性。
