...
| **可信度** | **AVM RI** | **Conformal Prediction**（split-CP→EnbPI/ACI）| RI ad-hoc、無覆蓋保證、雙模型可同錯(②)| **replace 語意層**（RI 留對照）| 理論成熟 | ✅ | **P1**(split)→**P2**(EnbPI/ACI)|
...
| L4 | 精確多維 Wasserstein O(N³logN)| **Sinkhorn divergence**（熵正則 OT）| 降到 O(N²/ε)、保 OT 幾何、少量點友善(①③)| replace 量級指標 | 成熟(Genevay)| ✅ | **P1** |
...
- **How**：base estimator 仍 PLS/GPR，CP 只是**外掛校準層**——對骨架 surgical（加 calibration set + wrapper）。RI 保留為相容對照。
...
- **分工（Rule 7，不混成單一公式）**：**MMD 出 p-value（是否相異）、Sinkhorn 出可解釋距離（漂移多大）、PSI 供人看嚴重度帶**。
- **When**：P1 即可換主判據與量級指標。
...
| 階段 | 納入項 | 性質 |
|---|---|---|
| **Phase 1（MVP，即插/低成本、全確定性）** | DPCA、RBC、FastMCD、ruptures(PELT)、**Conformal split-CP**、**MMD + Sinkhorn**（L4 主判據+量級）| 介面相容、無需深度資料、立即提升 |
...
## 6. 風險與待補（Rule 12）
- **DOI 衝突**：L3 檔 Cheng 2008 RI 標 `10.1109/TSM.2007.914388`，與已查證 `literature_crossref.md` 的 `10.1109/TSM.2007.914373` 衝突 → **以 .914373 為準**，L3 檔待修。
- **NOT VERIFIED 待補正式 DOI**：工業時序 CP 2025(IEEE 10870871)、Unified JITL 2022、Spatio-temporal LSTM 2023、CVA incipient 2024、NF 時序 2023、TEP-GNN/Anomaly Transformer/Real NVP（會議無 DOI）。落地引用前須補查（嚴禁當已驗證引用）。
