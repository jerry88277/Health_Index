...
> **狀態（2026-07-03）**：M0–M10 已交付（五維鏈+HI+RBC+驗收+事件閉環+demo 5 步精靈+startup_manual；CV+ TDD-0、batch_features INC-1；全套 451 tests green）。現行計畫＝新精靈 batch-AVM 管線：INC-1[x] 疊圖+[param×stat] → INC-2 X* 上 DQIx/DQIy → INC-3 X*→映射模型(PLS/GPR+CV+)+X* GSI/T²/SPE(fresh) → INC-4 批次/機台/時間選取後端 → INC-5+ Dash 9 步精靈前端；風險項（TDD-3 隔離、G1 純 Y 模組、G2/G3 X 歸因、數值護欄）另排；SMTP/誤報指標暫緩。權威：batch_avm_design.md §10＋devlog 2026-07-03。

| M | 里程碑 | 主要交付 | DoD | 依賴 |
|---|---|---|---|---|
| M0 | 骨架/環境 | venv、config.py（單一超參表）、interface.py 雙軌契約、.gitattributes、套件骨架、`.claude` hook 已就緒 | `pip install -e .`、import 通過、空測試綠 | — |
| M1 | 資料層 | 實際落地：synthetic / tep（含 12 個真實 MMFDD .mat、tep.generate() 可跑）/ ccpp(+covert) / steel(+covert) / uci_gas_drift / indpensim adapters→契約；pyTEP 棄用（需 MATLAB engine，synthetic.py:3）、PRONTO 未落地；**ruptures 切段＋穩態 gate**；transition/maintenance 排除 | adapter 對照官方欄位語意；TEP 切出已知穩態段 | M0 |
...
```mermaid
flowchart LR
    G[synthetic adapter＋MMFDD-TEP 注入型 covert drift\n（明確標記非真實物理失效）] --> P[主驗證 AC-1/2/3]
    P --> X{同邏輯，僅版本化超參}
    X --> D1[Extended TEP modes]
    X --> D2[CCPP/Steel 真實集（uci_gas_drift 純 drift 壓測）]
    X --> D3[Gas Drift 純 drift]
    D2 & D3 --> R[【真實集不退化】=正式採用門檻]
```
- synthetic＋MMFDD 注入型 covert drift 是合成 oracle → **正式採用須綁真實集(CCPP/Steel/uci_gas_drift)不退化**，非只看合成 oracle（避免循環論證）；AC-4 真實集不退化現況＝backlog（validation/crossval.py:99 誠實標）。
...
- 分階段：**P1**=DPCA/RBC/FastMCD/ruptures/split-CP(有 Y 時；小 n 走 CV+/jackknife+，已建 detectors/conformal_cv.py，覆蓋誠實 ≥1−2α)/MMD-first-pass-KS；**P2**=SFA/CVA/ICAD/EnbPI·ACI/ADWIN·BOCPD/Isolation Forest；**P3**=DKL·SVGP/VAE/KPCA·ICA/GNN(遠期)。
- **界線註**：batch-AVM step-7 映射模型已裁決「GPR+PLS+split-CP、零新模型」（wf_c8367a17＋雙紅隊；RF/GBM/XGB/KRR/SVR/MLP 皆 SKIP）——本 P2/P3 清單僅適用偵測鏈現代化候選，不得作為映射模型換模依據。
...
開放缺口（2026-07-02 對抗式風險稽核 wf_8bac24a4，10 agents）：SMTP 串接、Y 事件→X 歸因（G2）、G3 適用域定義（現僅 X 側 T² proxy）、headless 即時 runner、數值護欄（T² 求逆/RBC 除零→錯歸因）、跨線多重比較、TDD-3 結構隔離測試、operator 回饋迴路；其中 SMTP/誤報指標為使用者定調暫緩、非遺漏。
