...
6c. **[x] TDD-7 殘餘：模型比較 CV harness（新模組，非 crossval.py）**：`validation/cv_harness.py`（§13 #5 落地；out-of-fold 無洩漏比較 + held-out conformal coverage 稽核 + nested 無偏選擇；紅隊修掉 repeats no-op）。7 測試綠。
...
8. **[x] 非 code**：Barber crossref VERIFY **已完成**（§13 #6，DOI 10.1214/20-AOS1965 + Project Euclid 頁碼 486–507，已入 `literature_crossref.md`、`conformal_cv.py` 標記改 VERIFIED）；DQIy DOI 衝突**已解**（統一 `10.1109/TSM.2011.2146006`，見 §11）；dcc.Interval 進度基建**已落地**（INC-5，見 §11 Gate2）。
...
9. [x] 批次生命週期 runtime（10min 起監 X→2h Ŷ_middle→4h Ŷ_final→出 Y 查 G1）——`batch_avm/lifecycle.py`：`BatchSchedule`/`due_phases`（時刻表）+ `LifecycleState`（completed/g3_notified，JSON 存載 resume-safe）+ `run_phase`（相位狀態機）。`x_monitor` 僅監看 X 域不發信；`yhat_middle`/`yhat_final` 算 Ŷ + 正式 G3 AD，越域→通知且**同批只發一次**（防重複告警/重複扣費）；`y_measured` 事件驅動，Y 出爐才判 G1（**結構上不可提前**——Ŷ 相位絕不出 G1），觸發則接 G2 的 X 歸因。G1×G3 同批＝**兩封獨立信**（#8）：先前已發 G3 者只交叉引用 `co_g3`、不重送；未發過則於此點與 G1 同時共發兩封。**誠實標記（Rule 12）**：`x_monitor`/`yhat_middle` 的 X* 來自部分批、與 golden 全批 X* 不可交換 → `partial_basis=True` + caveat，屬早期預警非已校準告警（要嚴格校準需另建部分批相位模型，Rule 2 暫不做）。8 WHY 測試綠、隔離測試綠、全套綠。
10. [x] 殘差自相關 ARIMA 白化——`batch_avm/whiten.py`：`ljung_box`（自相關檢定）+ `fit_whitener`（AIC 選階，共同樣本可比）+ `whiten`（innovations）；接進 `residual.py` 的 G2 監控（fit 時白化 golden 殘差建 null、score 時白化線上殘差）。**為何需要**：CUSUM/EWMA 的界假設獨立觀測，殘差正自相關→等效樣本縮水→誤報膨脹；`YHistoryMonitor` 的經驗 h 只吸收**尺度**、吸收不了**相依結構**。**兩個誠實決定（Rule 7/12）**：(1) 實作 **AR(p) 而非完整 ARIMA**——殘差本應平穩（差分 I 會抵消漂移＝本監控的反面）、MA 對批級屬過度設計、statsmodels 不在技術棧，故用 numpy OLS + scipy chi2 保確定性零新依賴，偏離 backlog 用詞的理由明標於模組 docstring；(2) **資料驅動不強制**——Ljung-Box 不顯著就不白化（order=0 identity，不對沒壞的東西轉換、不白丟前 p 點、向後相容）。NaN 缺口→innovation NaN（不推補）。7 WHY 測試綠含**閘門校準鎖**（iid 拒絕率≈名目 5%、AR(1) 檢定力≥0.9——閘門偏誤會造成過度/不足白化）、既有 residual 6 測試與隔離綠、全套綠。
11. [x] mindmap v3（架構圖更新至 batch-AVM）——`docs/architecture_mindmap.html`：抬頭標 v3·2026-07 + 北極星（多產線儀表板）敘述、新增 **batch-AVM 路徑流程條**；新增兩個主幹——**「batch-AVM 路徑」**（為何另闢一軌/隔離契約、X* 轉換、映射+CV+、G3 正式適用域、殘差監控+AR 白化、G2/G3 歸因、同質性閘、headless runner、批次生命週期）與**「三目標與通知」**（G1 純 Y-vs-歷史、G2、G3、G1×G3 兩封信優先規則）；介面層補「多產線總覽(北極星)」與「batch-AVM 9 步精靈」；驗證補「CV harness」；「範圍與現況」更新為 569 測試綠 + 誠實限制清單（SMTP 未串、驗收指標未定、machine_id rollup 未做、部分批門檻未校準、TEP 端到端未實測、UI 互動未驗證）。瀏覽器實載驗證：81 節點渲染、10 個新節點全在、**零 console error**。
