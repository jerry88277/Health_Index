...
> 狀態註記（2026-07）：本檔為 modernization 階段 ledger（歷史紀錄）。三條「下一步」均已落地（drift.py KS first-pass→MMD 成本分層＝M5；development_plan.md §3 出口條件/§4 節拍預算＝v0.2；H7 經 reconciliation 定讞 ❌/🔴）；split-CP 冷啟動結論其後被 CV+/jackknife+（2026-07-02）部分超越；本檔早於 2026-07 產品重聚焦（多產線儀表板/G1–G3/新精靈）。
...
2. **丟掉 KS 是淨負債，不是淨升級**。KS 在 1D-on-PCA-score 上有解析 p-value、零 permutation、零調參。換 MMD 後每窗多出 `O(B·n²)` permutation + bandwidth/kernel 選擇 + seed 治理，全壓到**沒有統計博士的維運團隊**身上。audit H4/H8 承認 MMD 非全面碾壓，卻仍讓 KS「退場」——落地上應是 **MMD augment、KS 保留為廉價 1D 哨兵**（✅ 已採納：detectors/drift.py 實作 KS first-pass→MMD 分層＋成本分層，M5，devlog 2026-06-02）。
...
| **split-CP（可信度）** | calibration set 需**有標籤 Y**，但專案前提 Y 稀少（FR-3、§7）。小 calibration 下 empirical coverage 分佈很寬，單一 split 可能遠低於名目（已查證，見 §5）。要 α=0.1 且「95% 把握達 90% coverage」級別保證，calibration 需數百–上千筆有效樣本——**地端冷啟動期根本湊不到**（部分被超越：2026-07-02 新增 CV+/jackknife+（detectors/conformal_cv.py），自有門檻 cv_plus_min_obs=20、誠實覆蓋 ≥1−2α=0.80@α=0.1——冷啟動期已可給弱保證區間；「單一 split 小 calibration 不可靠」的統計論點本身仍成立）。 | ②③④ | 🔴 | 採 audit H1 的修正（GSI/T² 擔無標籤可信度，CP 僅在累積足夠 lab-Y 後上線）；明訂 CP 上線的最小 calibration 門檻並版本化 |
...
| **B3** | **permutation/Sinkhorn 的線上節拍預算從未被量化**（部分落地：development_plan.md §4 已列節拍預算 NFR 與降級規則（MMD→KS、Sinkhorn→1D-W）；具體數字（窗大小/節拍上限/每指標時間預算）仍未定） | Rule 6 要求「超節拍 surface」，但無人給**目標節拍數字**（每窗幾秒？窗多長？n 多大？）。沒有預算就無法判斷 MMD permutation 是否扛得住。 | 在 `development_plan.md` 補一條 NFR：定義線上窗大小、節拍上限、各 L4 指標的 per-window 時間預算與 fallback |
...
> **下一步**
> - 將 H7 從 🟢 升級為 🔴 並改寫為「permutation/CP 含 RNG，需鎖 seed＋容忍帶斷言＋審計記錄 B 與 seed，且優先用解析門檻」。（✅ 已落地：H7 經 redteam_reconciliation.md 定讞 ❌/取 🔴（C6）；解析門檻優先與鎖 seed＋容忍帶已進 v0.2 與測試慣例（M3 起 parametrize＋容忍帶；development_plan.md §7 RNG 風險列管）。modernization_audit.md 原文未回改，由 reconciliation 對帳表取代。）
> - L4 改「KS 廉價 1D 哨兵 → MMD 升級偵測」成本分層，撤回「KS 退場」。（✅ 已落地：detectors/drift.py：KS first-pass（解析 p-value、Bonferroni×維度）→ 觸發才升 MMD（block-permutation）；成本分層有 mutation 測試守護（M5）。）
> - 在 `development_plan.md` 補 B3（線上節拍預算 NFR）與 H2 出口條件（A/B 上限＋真實集綁定＋預設基線）。（✅ 已落地：development_plan.md v0.2：§3「現代化採用準則（紅隊 H2，含出口條件）」、§4「線上節拍與運算預算（紅隊 D3，新增）」、§5 超參治理（D1）、§7 風險表補 FWER/超參/RNG/自相關。）
